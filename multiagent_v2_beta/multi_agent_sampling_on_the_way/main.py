...
if __name__ == "__main__":
    ...
    parser.add_argument("--osp", help="On the way sampling parameter", default=0.05, type=float)
    ...
    for i in range(args.n_agents):
        agents.append(SOW_agent(mesh=meshgrid, env_sample=env_sample,
    ...
    for _ in range(args.n):
        ...
        init_cor, back_cors = get_cors(meshgrid, agents, args.beta)
        plot(env_sample, meshgrid, agents, _, directory)
        plot_sigma(meshgrid, agents, _, directory)
        for i in range(len(agents)):
            print(len(back_cors[i]))
            l = np.arange(0, len(back_cors[i]), int(len(back_cors[i])/(args.osp*len(back_cors[i]))))
            back_cors[i] = np.array(back_cors[i])[l]
            for b in back_cors[i]:
                print("Agent: "+str(agents[i].id)+" "+str(b))
                agents[i].learn(b)
