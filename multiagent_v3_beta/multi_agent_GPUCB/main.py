...
def get_cors(meshgrid, agents, beta):
    ...
    mu = torch.tensor(meshgrid.mu.copy(), requires_grad=False)
    sigma = meshgrid.sigma.copy()
    print((np.min(sigma), np.max(sigma)))
    sigma = torch.tensor(np.where(sigma==0, 1e-15, sigma), requires_grad=False)

    entropies = Normal(mu, sigma).entropy().numpy()

    idx = []
    cor = []
    for _ in range(len(agents)):
        idx.append(np.argmax(entropies))
        entropies[idx[-1]] = np.NINF
        cor.append(grid[idx[-1]])
        for __ in range(2000):
            entropies[np.argmax(entropies)] = np.NINF
